...
class DataLoader:
    ...
    def load_transform_file(self, file_path):
        ...
        # Load data into Neo4j using the CSV file we created
        with self.driver.session() as session:
            # Create constraints for better performance
            session.run(
                "CREATE CONSTRAINT location_id IF NOT EXISTS FOR (l:Location) REQUIRE l.name IS UNIQUE"
            )
    ...
    while attempt < total_attempts:
        try:
            data_loader = DataLoader(
                "neo4j://localhost:7687", "neo4j", "graphprocessing"
            )
